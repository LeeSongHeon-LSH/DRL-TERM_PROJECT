# Vendored verbatim from SkyworkAI/skywork-o1-prm-inference/model_utils/modeling_base.py (Apache 2.0).
...
class PreTrainedModelWrapper(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if (is_loaded_in_8bit or is_loaded_in_4bit) and "device_map" not in pretrained_kwargs:
            logging.warning(
                "The `device_map` argument is not provided. Overriding to current device."
            )
        ...
        if isinstance(pretrained_model_name_or_path, str):
            ...
            raise ValueError(
                f"pretrained_model_name_or_path should be a str or PreTrainedModel, got {type(pretrained_model_name_or_path)}"
            )
            ...
            if is_resuming_training:
                if is_sharded:
                    ...
                    for shard_file in files_to_download:
                        ...
                        state_dict.update(loading_func(local_path, **load_kwargs))
                else:
            ...
            if os.path.exists(index_filename):
                ...
                try:
                    index_file_name = hf_hub_download(
                        pretrained_model_name_or_path, model_index_name, token=token
                    )
                except (EntryNotFoundError, LocalEntryNotFoundError, HFValidationError, RepositoryNotFoundError):
                    is_resuming_training = False
                    logging.warning(
                        f"A {type(pretrained_model)} model is loaded from '{pretrained_model_name_or_path}',"
                        " and no v_head weight is found. Expected if not resuming PRM training."
                    )
            if is_resuming_training:
...
def create_reference_model(
    ...
    if is_deepspeed_zero3_enabled():
        raise ValueError("DeepSpeed ZeRO-3 is incompatible with create_reference_model().")
    parameter_names = [n for n, _ in model.named_parameters()]
    ref_model = deepcopy(model)
    if num_shared_layers is None:
        for n in parameter_names:
            ref_model.get_parameter(n).requires_grad = False
        return ref_model.eval()
    if pattern is not None:
        pattern = pattern.format(layer=num_shared_layers)
    else:
        for cand in LAYER_PATTERNS:
            cand = cand.format(layer=num_shared_layers)
            if any(cand in n for n in parameter_names):
                pattern = cand
                break
    if pattern is None:
    ...
    for n, _ in model.named_parameters():
        if pattern in n:
            sharing = False
        (shared if sharing else unshared).append(n)
    for n in shared:
        model.get_parameter(n).requires_grad = False
    for n in unshared:
        ref_model.get_parameter(n).requires_grad = False
    return ref_model.eval()
