...
작성일: 2026-05-10
대상 계획서: `구현 계획 — 차분 PAV + 분포형 보상 (Phase 0).md`
...
│   └── rl_q3.yaml                 # 메인 RL 조건 (Q3, λ=-0.5, K=16)
├── src/
│   ├── prm/
│   │   ├── loader.py              # PRMConfig + load_prm
│   │   ├── score.py               # PRM(score, score_batch, score_per_step)
...
│   │   ├── mu_sampler.py          # μ.sample_step_batch (vLLM)
...
│   │   ├── grpo_trainer.py        # build_grpo_trainer (TRL GRPOTrainer)
...
```bash
uv sync                       # base
uv sync --extra gpu           # +vllm/trl/peft/wandb/math-verify
...
전체 분산 스택을 docker compose로 묶어 둠:
- `Dockerfile` — `pytorch:2.5.1-cuda12.4-cudnn9-runtime` + uv + extras=gpu (단일 이미지)
- `docker-compose.yml` — 4 services + profiles
  - `broker` (rabbitmq:3.13-management)
  - `prm` (prm-worker, GPU)
  - `mu` (mu-worker, GPU)
  - `trainer` (vLLM colocate, GPU, shm_size=8g)
- `.env.example` — AMQP_URL / HF_HOME_HOST / HF_TOKEN / WANDB_API_KEY

호스트 NVIDIA Container Toolkit이 깔린 상태에서 `docker compose --profile X up`만으로 각 PC에서 자기 service를 띄움. HF 캐시는 호스트 bind mount로 영속화.
...
- 현재 default는 **1.5B (~2.9GB)** — 단일 GPU에서 PRM + 정책 + μ를 동시에 띄워도 여유 있음.
  - `configs/prm.yaml`의 `model_id`를 `Skywork/Skywork-o1-Open-PRM-Qwen-2.5-7B`로 바꾸면 7B 변형 사용.
- AWQ 4-bit 공식 변환본 미제공. 7B를 fp16으로 RTX 4090(24GB)에서 띄워도 ~14GB로 충분.
...
- 원본 계획서: `구현 계획 — 차분 PAV + 분포형 보상 (Phase 0).md`
