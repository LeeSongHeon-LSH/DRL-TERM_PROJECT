# Quickstart — 2 PC 분산 (3090 학습 + cloud T4 추론)
...
- **같은 LAN** — 사설 IP로 직접 통신 (가장 단순). FRP 셋업 불필요 → 바로 [Section 1](#1-추론-pc-3090-ti--50xx-등--μ--prm-서빙) 진행.
- **다른 위치 / 학습 PC 공인 IP 보유, 추론 PC NAT 뒤** — FRP TCP tunnel 사용 → 아래 0-3) 먼저 진행.

### 0-3) FRP server 셋업 (FRPS host, FRP 모드 사용 시만)

**Section 1 (추론 PC) 에서 `FRPS_TOKEN`, `FRPS_ADDR` 입력이 필요한데 — 이건 FRPS host 에서 미리 frps 띄워야 발급됨**. 순서: FRPS host frps → 추론 PC frpc → 학습 PC trainer.

#### Step 1: 라우터 포트포워딩 + DDNS (FRPS host 기준)

- 라우터: **TCP 7000** → FRPS host LAN IP
- 방화벽: TCP 7000 inbound 허용
- (공유기 IP 자주 바뀌면) DDNS: DuckDNS / no-ip 등 무료. 예: `myhost.duckdns.org`

도달성 검증 (외부 LTE 등 **다른 네트워크에서**):
```bash
nc -vz <FRPS host 공인IP 또는 DDNS> 7000     # "succeeded" 떠야 OK
```
...
→ 이 시점부터 FRPS host frps 가 추론 PC frpc 접속 대기. Section 1 진행 가능.

---

## 1. 추론 PC — μ + PRM 서빙

### 1-A. Ampere / Ada / Turing GPU (3090 Ti, 4090, **cloud T4 16GB** 등) ⭐

> 본 실험 추론은 **cloud T4 (Turing, 16GB) ×N** — Turing은 PyTorch 2.5.1 지원이라 이 표준 경로(`docker-compose.inference.yml`)를 그대로 사용. 단 bf16 미지원 → **fp16**, `MU_GPU_MEM=0.6` 권장 (μ ~6GB + PRM int8 ~1.7GB ≈ 8GB / 16GB).
...
### 1-B. (옵션) Blackwell GPU (RTX 50-series)

> ⚠️ **본 실험 추론은 cloud T4(Turing)를 1-A 경로로 사용** — 이 섹션은 실제 Blackwell(RTX 50-series) 카드를 쓸 때만 필요. T4·Ampere·Ada는 1-A로 충분.
...
```
┌──────────────┐  HTTP  ┌──────────────┐  HTTP  ┌──────────────┐
│ 학습 PC 3090  │◄──────►│ 50xx PC #1   │       │ 50xx PC #2   │
...
**⚠️ Blackwell (RTX 50xx) 호환성:**
- RTX 50xx은 Blackwell 아키텍처(SM 12.0)입니다.
...
# μ 전용 50xx PC (vLLM 공식 이미지, 빌드 불필요):
FRPS_ADDR=<학습PC 공인IP/DDNS> FRPS_TOKEN=<frps 와 동일 토큰> NODE_NAME=50xx-mu-01 \
  docker compose -f docker-compose.inference-blackwell.yml up -d mu-server

# PRM 전용 50xx PC (Dockerfile.blackwell 빌드):
FRPS_ADDR=<학습PC 공인IP/DDNS> FRPS_TOKEN=<frps 와 동일 토큰> NODE_NAME=50xx-prm-01 \
...
### 옵션 B: FRP TCP tunnel (FRPS host 공인 IP, 추론 PC NAT 뒤)

`.env`에 frps endpoint 설정:
```bash
# 로컬 frps 컨테이너를 쓸 때 (default)
...
# http://<FRPS_PUBLIC_IP_OR_DNS>:7500 (admin / FRPS_DASHBOARD_PW)
```

VRAM 점유: **~18 GB / 24 GB** (π 1.5B Full FT + 8bit AdamW(`adamw_bnb_8bit`) + vLLM colocate 0.30 = 7.2 GB).
...
pav:
  ...
  K: 16
grpo:
  learning_rate: 2.0e-6            # 1.5B Full FT 스케일 (5e-7 → 2e-6 상향)
  group_size: 8                    # GRPO trajectory 수 (문제당 rollout)
  warmup_steps: 50                 # 5000 step의 1%
  optim: adamw_bnb_8bit            # ⭐ 현재 default — bitsandbytes 8-bit AdamW (VRAM 절반)
                                   # galore_adamw_8bit_layerwise는 단일 PC swap 경로(rl_q3_swap.yaml)에서 사용
                                   # paged_adamw_8bit은 step scratch 12GB로 OOM, CAME은 RAM 폭주 hang (둘 다 회피)
vllm:
  colocate: true
  gpu_memory_utilization: 0.30     # 7.2 GB — KV cache 여유, π rollout 안정
  max_new_tokens: 256              # = 학습 max_completion_length
```
...
| `MU_GPU_MEM` (추론 cloud T4 16GB) | 0.6 | μ KV cache ~9.6 GB (n=16 batch 안정) |
...
50 step 실측: **~54분** (step time 60-90초).
...
| `grad_norm` | parameter gradient의 L2 norm (clipping 전) | 정상: 0.1 ~ 10. 메인 default `adamw_bnb_8bit`는 정상 reporting (실측 0.4~0.9). **0.0이면** ① (swap 경로의) GaLore layerwise는 fused norm을 reporting 안 함 (artifact) ② grad clipping 작동 ③ 진짜 grad 거의 없음 — checkpoint diff로 확인 |
| `learning_rate` | 현재 step의 LR (cosine schedule 적용 후) | `2e-6` (initial) → `0` (final). `warmup_steps=50`까지 ramp-up, 이후 cosine decay |
...
## 8. FRP TCP tunnel cluster (cloud T4 × N대 추론, 학습 PC 공인 IP 활용)
...
# 노드 고유 라벨 inline 전달
FRPS_ADDR=myhost.duckdns.org FRPS_TOKEN=<학습 PC 와 동일 토큰> NODE_NAME=t4-pc-01 \
  DOCKER_BUILDKIT=1 docker compose -f docker-compose.inference.yml up -d --build
```
